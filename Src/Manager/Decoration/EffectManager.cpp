...
void EffectManager::Play(const EFFECT& _efc, const VECTOR& _pos, const VECTOR& _qua, const float& _size, const SoundManager::SOUND _sound)
{
	//元データがないときは警告
...
void EffectManager::SyncEffect(const EFFECT& _efc, const VECTOR& _pos, const VECTOR& _qua, const float& _size)
{
	//その他各種設定
	//大きさ
	SetScalePlayingEffekseer3DEffect(effectPlay_[_efc], _size, _size, _size);
	//角度
	SetRotationPlayingEffekseer3DEffect(effectPlay_[_efc],_qua.x, _qua.y, _qua.z);
	//位置
